...
This package was developed using data from a Netzsch TGA-FTIR thermogravimetric analyser. We have included three example datasets in this package, `beech`, `juncus`, and `marsilea`. The `beech` dataset contains thermogravimetric data for a sample of beech wood that underwent a non uniform heating program over time; `juncus` and `marsilea` contains theromogravimetric data for a sample of the freshwater reed, *Juncus amabilis*, and a freshwater fern, *Marsilea drumondii*, respectively, over a uniform heating program. 

``` {r}
library(mixchar)
plot(beech$time, beech$temp_C)
plot(marsilea$time, marsilea$temp_C)
```
...
After we've loaded our data, can use the function `process()` to take the derivative of the pyrolysis phase, resulting in rate of mass loss over temperature data. This produces the multi-peaked rate of decay curve that we will apply the mixture model to. The `process` function needs the dataset, the initial mass of sample, the name of the temperature data column, the name of your mass loss column, the name of the time column, and the start and end times for the pyrolysis phase. 
...
```{r}
processed_beech <- process(
  beech,                                # dataframe name
  init_mass = 10.64,                    # initial mass of sample
  temp = 'temp_C',                      # col name for temperature data
  mass_loss = 'mass_loss',              # col name for mass loss data
  time = 'time',                        # col name for time data
  pyrolysis_start_time = 127,           # time at which pyrolysis phase began
  pyrolysis_end_time = 191.5,           # time at which pyrolysis phase ended
  temp_units = 'C')                     # temperature units
processed_beech
```
...
deriv_marsilea <- process(marsilea, 
                          ...
                          mass_loss = 'mass_loss', 
                          time = 'time',
                          pyrolysis_start_time = 5295,
                          pyrolysis_end_time = 37109)
