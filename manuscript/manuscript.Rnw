...
<<echo = FALSE, fig.cap = '\\label{Fig:fs_simulation} Parametric examination of Fraser-Suzuki function using simulated data', out.width = '.9\\linewidth', fig.asp = 1>>=
...
<<fig.cap = '\\label{Fig:decon} Deconvolution of \textit{Juncus amabilis} example dataset. Mass loss data overlaid with output of deconvolution. Rate of mass loss scaled by initial mass of sample.'>>=
plot(output_juncus)
@
