...
After we've loaded our data, we need to calculate the rate of mass loss by taking the derivative. The \verb|process| function needs the dataset, the initial mass of sample, the name of the temperature data column, and the name of the mass column (mg), which can be specified either as mass loss data with the \verb|mass_loss| argument or as mass data with the \verb|mass| argument.
...
The output of the \verb|deconvolve| function is a list of five items that can be retrieved with accessor functions:
\begin{enumerate}
\item{the processed dataset that results from the \verb|process| function, useful if for playing around with other modelling approaches or plotting options, and accessed with \verb|rate_data()|:
<<>>=
...
\item{the output of the mixture model and estimated parameter values for height, skew, position, and width for each peak. Peak 1 is hemicellulose, peak 2 is cellulose, and peak 3 is lignin. If present, the optional fourth peak located at the lowest temperature interval will be listed as peak 0. Accessed with \verb|model_fit()|:
<<eval = FALSE>>=
...
Plotting the output of the \verb|deconvolve| function shows the underlying DTG data, the overall mixture model curve, as well as the components peaksof the deconvolution (Fig.~\ref{Fig:decon}). The default plot is in black and white, but you can plot a colour version that uses colour-blind friendly \verb|viridis| colours \citep{viridis} by specifying \verb|bw = FALSE|.
...
The Fraser-Suzuki family of functions are exported to allow users to create their own plots from the model outputs. You can use these in conjunction with the parameter estimates, accessed as follows:
...
Something about passing CRAN check with no errors or warnings, or notes.

All the functions of \verb|mixchar| were tested to see if they produced the desired output. The workflow was tested on thermogravimetric data from two different TGA-FTIR machines, and on samples outside those used to build the package.



\textit{Seagrass blade}
<<echo = FALSE>>=
source('stt_analysis.R')
sg_leaf <- process_data('data/seagrass_leaf.txt')
@


after loading the data, and using the process function, we can inspect the plots:
<<>>=
plot(sg_leaf)
@

we can determine that this sample needs a fourth peak, and the lower and upper bounds modified.
<<>>=
sg_out <- deconvolve(sg_leaf,
                     n_peaks = 4,
                     lower_temp = 150,
                     upper_temp = 600)
plot(sg_out, bw = FALSE)
@
the outcome looks good.

\textit{Seagrass rhizome}
similarly needed 4 peaks specified and bounds.
<<echo = FALSE>>=
sg_rhiz <- process_data('data/seagrass_rhizome.txt')
@

<<>>=
sg_rh_out <- deconvolve(sg_rhiz,
                        n_peaks = 4,
                        lower_temp = 150,
                        upper_temp = 600)
plot(sg_rh_out, bw = FALSE)
@


\textit{Seagrass root}
<<echo = FALSE>>=
sg_root <- process_data('data/seagrass_root.txt')
@

<<>>=
sg_root_out <- deconvolve(sg_root,
                          lower_temp = 150,
                          upper_temp = 600)
plot(sg_root_out, bw = FALSE)
@

\textit{Macroalgae}
<<echo = FALSE>>=
ma_leaf <- process_data('data/macroalgae_leaf.txt')
@
...
ub <- c(2, 0.2, 260, 80,
        ...
        2, 0.2, 380, 50,
        2, 0.2, 430, 250)
ma_out <- deconvolve(ma_leaf,
                     n_peaks = 4,
                     start_vec = start_vec,
                     upper_vec = ub,
                     lower_temp = 150,
                     upper_temp = 600)
plot(ma_out, bw = FALSE)
@
...
\begin{description}[noitemsep,topsep=0pt]
	\item[Name:] Github \url{https://github.com/smwindecker/mixchar/}
...
For those that wish to contribute to the package, it is hosted on Github. Contributors can log issues, for example with alternative data foramts, via the issues tracker (\url{https://github.com/smwindecker/mixchar/issues}) or submit a pull request to add functionality to the package.

\section*{Acknowledgements}
The authors would like to thank Dr. Nick Tierney and David Wilkinson for reviewing an early version of the package as well as CRAN reviewers for useful additions. We would like to thank volunteers Paula Sanchez, Abbey Kinnish, Kelsey Johnson, Madeline Brenker, and Urtzi Enriquez Urzelai for assistance in the field collecting plant specimens. We would like to thank the University of Melbourne Department of Chemical and Biomolecular Engineering for access to and training on the TGA-FTIR. Vegetation collection for data used to build and verify the model was conducted under Victorian Department of Environment, Land, Water and Planning Permit No 10007429.
